...
class AbstractDeviceTest(TestCase):
	...
	# skip this test? doesn't raise DeviceNotFoundError
	# instead raises OSError ('1234' not valid) and TimeoutError
	# (PyVISA times out attempting to connect to '192.0.2.123')
	...
	def testInitNotFoundGPIB(self):
		...
		except abstract_device.DeviceNotFoundError as AttributeError:
			pass
		else:
			assert False, 'Expected DeviceNotFoundError.'
