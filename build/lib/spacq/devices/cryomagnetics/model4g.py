...
class Model4G(AbstractDevice):
    ...
    def reset(self):
        ...
        for channel in [1,2]:
            ...
            resource_list =       [ subdev.virt_energysave_mode,
                                    subdev.virt_sync_currents,
                                    self.virt_both_units,
                                    subdev.power_supply_current,
                                    subdev.magnet_current,
                                    subdev.persistent_switch_heater,
                                    subdev.high_limit,
                                    subdev.low_limit,
                                    subdev.virt_imag_sweep_to,
                                    subdev.virt_iout_sweep_to
                                    ]
            
            for resource, value in zip(resource_list, resource_check_list):
                if resource != value:
